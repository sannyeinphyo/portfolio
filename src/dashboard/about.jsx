...
import {
  SiNextdotjs, SiMysql, SiTailwindcss, SiBootstrap, SiFramer,
  SiJavascript, SiPrisma, SiHtml5, SiCss3, SiFigma,
  SiMongodb, SiPostman, SiMui, SiAngular, SiPostgresql,
} from "react-icons/si";
...
const About = () => {
  ...
  return (
    <div className="min-h-screen relative flex items-center justify-center p-6 bg-slate-50 bg-white/70 dark:bg-gray-800/40 transition-colors duration-500 overflow-hidden">
      ...
      <motion.div
        className="relative z-10 max-w-5xl w-full space-y-12 p-8 md:p-14 rounded-[2rem] mb-24 md:my-12
      ...
      >
        {/* Title */}
        <motion.h1 variants={fadeUp} className="text-5xl md:text-6xl font-black text-center tracking-tight text-slate-900 dark:text-white">
          About <span className="text-indigo-600 dark:text-blue-400">Me.</span>
        </motion.h1>

        <motion.p variants={fadeUp} className="text-lg md:text-xl text-slate-600 dark:text-gray-300 text-center leading-relaxed max-w-3xl mx-auto">
          Hi, I'm <span className="font-bold text-slate-900 dark:text-white">San Nyein Phyo</span> — a full-stack developer turning complex problems into elegant, functional code.
        </motion.p>

        {/* Technical DNA Section - IMPROVED */}
        <motion.div variants={fadeUp} className="flex flex-col md:flex-row gap-2 p-2 rounded-[2.5rem] bg-slate-100/80 dark:bg-white/5 border border-slate-200 dark:border-white/10">
          {[
            { label: "Frontend", value: "Angular / React", desc: "Interactive & Modern UIs" },
            { label: "Backend", value: "Node.js / Express", desc: "Secure & Scalable Logic" },
            { label: "Database", value: "Postgres / MySQL", desc: "Optimized Data Flow" },
          ].map((item, i) => (
            <div key={i} className="flex-1 text-center p-6 rounded-[2rem] transition-all duration-500 hover:bg-white dark:hover:bg-white/10 hover:shadow-xl hover:shadow-indigo-500/5 group">
              <p className="text-[10px] uppercase tracking-[0.3em] text-indigo-500 dark:text-blue-400 font-black mb-2 opacity-70 group-hover:opacity-100">{item.label}</p>
              <h3 className="text-lg font-bold text-slate-800 dark:text-slate-100">{item.value}</h3>
        ...
        <motion.div variants={containerVariants} className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-4">
          {skills.map((skill, idx) => (
            <motion.div
              ...
              className="bg-white/50 dark:bg-white/[0.03] backdrop-blur-md p-6 rounded-3xl border border-white dark:border-white/10 shadow-sm hover:shadow-md transition-all flex flex-col items-center gap-3"
            >
        ...
        <motion.div variants={fadeUp} className="text-center pt-6">
          <Link to="/contact" className="inline-block px-10 py-4 bg-slate-900 dark:bg-white text-white dark:text-slate-900 rounded-2xl font-bold shadow-lg hover:shadow-indigo-500/20 transition-all hover:-translate-y-1">
            Let's Connect
          </Link>
        </motion.div>
      </motion.div>
...
const StatusBadge = ({ text, fadeUpVariant }) => (
  <motion.div variants={fadeUpVariant} className="flex items-center gap-3 bg-white/80 dark:bg-white/5 px-4 py-2 rounded-full border border-slate-200 dark:border-white/10 backdrop-blur-sm w-fit shadow-sm">
    <span className="relative flex h-2 w-2">
      <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75"></span>
      <span className="relative inline-flex rounded-full h-2 w-2 bg-emerald-500"></span>
    </span>
    <p className="text-[11px] font-medium text-slate-500 dark:text-gray-400">
      Learning <span className="text-slate-900 dark:text-slate-200 font-bold">{text}</span>
    </p>
  </motion.div>
);
